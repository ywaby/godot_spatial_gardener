...
func volume_get_stroke_update_changes(brush_data:Dictionary, plant:Greenhouse_Plant, plant_index:int, octree_manager:MMIOctreeManager, 
	...
	# But I turned checked placement amount limiter and it looks surprisingly fine
	...
	for raycast_position in raycast_positions:
		...
		phys.from = raycast_position[0]
		phys.to = raycast_position[1]
		var ray_result = space_state.intersect_ray(phys)
